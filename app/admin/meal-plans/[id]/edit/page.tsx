...
import { DailyMealPlanFormData, Meal, Drink, DailyMealPlanRecord } from "../../../../../models/mealPlan"

export default function EditDailyMealPlanPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  async function fetchMealPlan() {
    try {
      ...
      if (data.breakfast) setValue("breakfast", data.breakfast)
      if (data.lunch) setValue("lunch", data.lunch)
      if (data.dinner) setValue("dinner", data.dinner)
      if (data.morningSnack) setValue("morningSnack", data.morningSnack)
      if (data.afternoonSnack) setValue("afternoonSnack", data.afternoonSnack)
    } catch (err: any) {
  ...
  const onSubmit = async (data: DailyMealPlanFormData) => {
    try {
      ...
      setTimeout(() => {
        router.push(`/admin/meal-plans/${id}`)
      }, 1500)
    } catch (err: any) {
      setError(err.message)
    } finally {
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              Update meal plan for {new Date(mealPlan.date).toLocaleDateString("en-US", { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
            </p>
          </div>
          <div className="flex gap-3">
            <Link
              href={`/admin/meal-plans/${id}`}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="flex justify-end gap-4">
            ...
            <button
              type="submit"
              disabled={saving}
              className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-semibold transition-colors disabled:opacity-60"
            >
